...
echo "==> Instalando dependencias (tensorflow, pandas, numpy, scikit-learn)..."
python3 - <<'PY'
...
echo "==> Descargando dataset SMS (tsv) si no existe..."
if [ ! -f sms.tsv ]; then
  wget -q https://raw.githubusercontent.com/justmarkham/scikit-learn-videos/master/data/sms.tsv -O sms.tsv || true
fi

echo "==> Creando sms_classifier.py..."
cat > sms_classifier.py <<'PY'
import os
import sys
import argparse
import numpy as np
...
echo "==> Entrenando modelo (rápido, ~8 épocas en CPU)..."
python3 sms_classifier.py --train --epochs 8

echo "==> Predicciones de ejemplo:"
python3 sms_classifier.py --message "Free entry winner! Claim your prize now by texting WIN to 80086."
python3 sms_classifier.py --message "Hey! I'll call you after the meeting."

echo
echo "==> Listo. Usa:"
echo "    python3 sms_classifier.py --message \"<tu mensaje>\""
echo "o  python3 sms_classifier.py --train --epochs 12   # (reentrenar)"
